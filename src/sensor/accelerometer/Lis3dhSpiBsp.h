 ...
 */
struct ILis3dhSpiBspListener {
        virtual ~ILis3dhSpiBspListener () {}
 ...
 * board / µC.
 */
class Lis3dhSpiBsp : public ILis3dhBsp {
public:
        Lis3dhSpiBsp (Spi &spi) : spi (spi), lis3dshBspListener (nullptr)
        {
                if (instance) {
                        Error_Handler ();
                }
        ...
        virtual ~Lis3dhSpiBsp () {}

        static Lis3dhSpiBsp *singleton () { return instance; }

        void itConfig ();
        void write (uint8_t *buffer, uint8_t addr, uint16_t len);
        void read (uint8_t *buffer, uint8_t addr, uint16_t len);

        void setLis3dhBspListener (ILis3dhSpiBspListener *l) { lis3dshBspListener = l; }
...
private:
        ...
        Spi &spi;
        ILis3dhSpiBspListener *lis3dshBspListener;
        static Lis3dhSpiBsp *instance;
};
