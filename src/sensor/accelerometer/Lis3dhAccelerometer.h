...
public:
        Lis3dh (ILis3dhBsp &bsp, Lis3dhInit const &initStruct);
        virtual ~Lis3dh () override = default;
        static Lis3dh *singleton () { return instance; }

        void reset ();
        uint8_t getId () const;
        AData getAData () const override;
        int8_t getTemperature () const;
        bool isDataReady () const; /// Checks for interrupt status DRDY

        // Config
        void interruptConfig (Lis3dhInterruptConfig const &LIS3DSH_IntConfigStruct);
        void clickITConfig ();
